...
log_filename = (
    f"/app/logs/motto-fleet-burn-rate-{datetime.now().strftime('%Y%m%d')}.log"
)
logging.basicConfig(
    level=logging.INFO,
...
MOTTO_FINANCE_API_URL = os.getenv(
    "MOTTO_FINANCE_API_URL", "https://motto-finance-tracker.onrender.com/api"
)
...
class MaritimeFleetAgent:
    ...
    def _safe_api_call(self, cost_usd: float, func, *args, **kwargs) -> Any:
        ...
        try:
            if self.cost_tracker + cost_usd > MAX_COST_PER_RUN:
                raise RuntimeError(
                    f"Cost limit exceeded: {self.cost_tracker + cost_usd:.2f} > {MAX_COST_PER_RUN}"
                )
    ...
    def _aggregate_costs(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for _, row in df.iterrows():
            if row["cost_usd"] > 1.2 * (
                df["cost_usd"].rolling(30, min_periods=1).mean().iloc[-1] or 0
            ):
                anomalies.append(
                    {
                        "vessel": row["vessel"],
  ...
  "risk_level": "LOW|MEDIUM|HIGH"
    ...
    def run_agent(self, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2. Aggregate costs
            summary = self._safe_api_call(0.5, self._aggregate_costs, df)

            # 3. Generate LLM analysis
            analysis = self._safe_api_call(1.0, self._generate_analysis, summary)

            # 4. Push results to motto-finance-tracker
            payload_full = {
                "run_date": datetime.now(self.utc).isoformat(),
                "summary": summary,
                "analysis": analysis,
            }
            push_success = self._push_to_motto_finance(payload_full)

            # 5. Record agent run
            self._write_agent_run(summary, push_success)

            duration = (datetime.now(self.utc) - start_time).total_seconds()
            logger.info(
                f"Fleet burn rate analysis complete in {duration:.2f}s, cost=${self.cost_tracker:.2f}"
            )
            ...
            self._send_alert(f"Agent run failed: {e}")
            result["error"] = str(e)
